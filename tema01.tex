...
%
% El argumento opcional de marginfigure es un desplazamiento vertical y aquí
% deja aire por arriba; el \vspace* final lo deja por abajo y, al contar para
% la altura de la caja, hace que LaTeX reserve el hueco y que ninguna nota al
% margen se pegue al retrato.
%
% El signo de tanto por ciento tras el corchete no es opcional: sin él, el
% salto de línea deja un espacio literal al principio de la caja y la imagen
% se sale 2,33 pt del margen (justo un espacio interpalabra del cuerpo del
% pie). Sin corchete no ocurre, porque el lector de argumentos opcionales de
% LaTeX se come los espacios mientras busca el «[».
%
% Una figura que ocupa página entera se centra verticalmente por defecto, lo
% que deja un hueco enorme por encima y parece un descuido. La alineamos arriba.
\makeatletter
\setlength{\@fptop}{0pt}
\makeatother
...
no todas las frecuencias están igualmente presentes. Si estudiamos cómo
se reparte la energía emitida entre las distintas longitudes de onda
$\lambda$, y cómo cambia ese reparto a medida que
...
de acuerdo con el \textbf{principio de equipartición} ---que asigna la
misma energía media $k_BT$ a cada oscilador, con independencia de su
frecuencia---, llegó, con la
...
radiación de muy alta energía) $\rho$ crecería sin límite al disminuir
$\lambda$. Esto conduciría a la denominada ``catástrofe
ultravioleta'': al sumar la contribución de todas las longitudes de onda,
la energía total radiada resultaría infinita, y bastaría con asomarse a
la cavidad para recibir una cantidad ilimitada de energía. 
Este resultado revelaba inequívocamente un fallo en la descripción
clásica del sistema.

\begin{figure}
  \includegraphics[width=0.8\textwidth]{cuerponegro_planck_rayleigh-jeans_v1.pdf}
...
alcanzar la energía $h\nu$. Aquí está la clave: a longitudes de onda
cortas $h\nu\gg k_BT$, de modo que la energía térmica disponible no basta
para excitar esos osciladores, que permanecen así en su estado
fundamental y no contribuyen a la radiación. Es justo lo contrario de lo
que suponía la equipartición, y es lo que elimina la catástrofe
ultravioleta.
...
Einstein razonó que arrancar un electrón del material cuesta una energía
mínima $\Phi$, el \textbf{trabajo de extracción}, característica de cada
material, y que un solo cuanto debe aportarla por completo: hace falta por
tanto que $h\nu\geq\Phi$, lo que equivale a una frecuencia umbral
$\nu_0=\Phi/h$.
...
derecho, $h\nu$ es la energía de la radiación incidente, y el exceso
sobre $\Phi$ es lo que se lleva el electrón: si $h\nu<\Phi$ no se
...
la constante de Planck, $h$. Que dos experimentos tan distintos ---el
ajuste de la radiación del cuerpo negro y el efecto fotoeléctrico---
condujesen con alta precisión al mismo valor de una misma constante
parecía proporcionar solidez a la misteriosa teoría
cuántica.

\section{Hipótesis de de Broglie}
La explicación del efecto fotoeléctrico estableció el carácter
corpuscular de la radiación, en forma de fotones. 
Otros experimentos, de naturaleza muy distinta, sirvieron para 
establecer la afirmación simétrica y aún más revolucionaria: el
comportamiento como onda de la materia. El experimento más importante lo llevaron a cabo 
...
¿Pero cómo racionalizamos esta dualidad de ondas y partículas?
...
y sus propiedades ondulatorias no pueden observarse
...
\begin{figure}
  ...
  cuatro objetos de masa muy distinta. El protón se toma a temperatura
  ambiente, moviéndose a su velocidad media de agitación térmica
  ($\approx2.5$ km/s a 300 K). La banda gris marca la escala atómica:
  solo se observa difracción cuando $\lambda_\mathrm{dB}$ es comparable al
  tamaño del obstáculo, y para una pelota es unos treinta órdenes de
  magnitud menor.}
  \label{fig:debroglie}
\end{figure}
...
con la mecánica clásica es el límite que impone a la determinación
simultánea de ciertas magnitudes. Lo recoge el principio de
incertidumbre, enunciado por
...
este principio en 
mayor profundidad más adelante en el curso, la dispersión de los
valores está limitada por
\begin{equation}
...
cualquier par de \textbf{variables complementarias}.
...
de un elemento como el hidrógeno, no encontramos una banda continua sino
un conjunto de líneas separadas: un \textbf{espectro de líneas}. La
emisión de un
átomo se produce, por tanto, solo a unos determinados valores de la longitud
...
Esto obligó a reformular los modelos atómicos en uso, incompatibles con
tal discontinuidad. El problema era más grave todavía: un electrón que
describe una órbita está acelerado y, según el electromagnetismo
clásico, debería radiar de forma continua, perder energía y precipitarse
sobre el núcleo en una fracción de segundo. Ni la estabilidad del átomo
ni el carácter discreto de su espectro tenían explicación.
...
\begin{enumerate}
    ...
    correspondientes a órbitas circulares en las que la atracción
    electrostática entre el núcleo y el electrón proporciona la fuerza
...
\begin{equation}
    ...
    \end{split}
\end{equation}
Para el hidrógeno, esta descripción es satisfactoria. Combinando la
expresión de $E_n$ con la condición de Bohr para una transición,
$\Delta E=hc\tilde{\nu}$, se obtiene la siguiente
expresión
\begin{equation}
...
$R_H=109677.58$ cm$^{-1}$ 
es la constante de Rydberg del átomo de hidrógeno, que el modelo predice
a partir de las constantes fundamentales. La Ecuación 
\ref{eq:Rydberg} explica las líneas espectrales de Lyman, Balmer, 
y Paschen, donde $n_1=1,2, 3$ y $n_2>n_1$, respectivamente
(Figura \ref{fig:bohr}).

El éxito del modelo de Bohr acaba, sin embargo, en el hidrógeno: aplicado
al helio o a cualquier átomo con más de un electrón, falla por completo.
Y ni siquiera para el hidrógeno explica por qué unas transiciones son más
intensas que otras. El acierto de la cuantización convivía así con la
arbitrariedad de imponerla sobre una mecánica clásica que seguía intacta.
Hará falta una teoría construida desde cero, y es la que desarrollaremos
